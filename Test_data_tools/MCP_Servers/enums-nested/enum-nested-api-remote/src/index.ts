...
function createCustomerProfileMultiLevel(args: any): any {
    ...
    let contactPreferenceValid = true;
...
app.get("/health", (req: Request, res: Response) => {
    res.json({
        ...
        service: "enum-nested-api-remote",
        transport: "HTTP",
    });
});
...
app.get("/", (req: Request, res: Response) => {
    res.json({
        message: "Enum Nested Schemas API - Remote MCP Server",
        version: "1.0.0",
        transport: "HTTP",
        endpoints: {
            health: "/health",
        ...
        documentation: "Direct implementation for enum validation in nested structures",
    });
});
...
// Start server
app.listen(PORT, () => {
    console.log(`Enum Nested API Remote MCP Server running on port ${PORT}`);
