...
Email stored as a string field for flexibility.
...
| **Enum References** | `customer.entityType` → `EntityType` enum | `person.department` → `DepartmentEnum` |
...
1. **Reference Types**:
   - Customer Order: Cross-schema references (address reuse) + enum reference (EntityType)
   - Employee Registration: Self-referential circular reference + enum reference (DepartmentEnum)
...
3. **Validation Complexity**:
   - Customer Order: Basic field validation + enum validation
   - Employee Registration: Pattern matching + enum validation

4. **Enum Usage**:
   - Customer Order: `EntityType` enum for customer classification (INDIVIDUAL/BUSINESS)
   - Employee Registration: `DepartmentEnum` for department classification (8 departments)
